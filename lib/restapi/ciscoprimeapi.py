...
class CiscoPrimeApi(lib.restapimaster.RestApi):
    ...
    def read_page(self, url, first_result=0, max_result=99, show=True):
        self.urlbase = url
        self.url_paged = self.urlbase + "&.maxResults=" + str(max_result) + "&.firstResult=" + str(first_result)
    ...
    def read_unreachable(self):
        ...
        while not self.reach_page_end:
            self.current_page = self.read_page(self.urlbase, self.first_result, self.max_result)
